...
export class StripeComponent extends BaseComponent {
  ...
  authorize() {
    ...
    return new Promise(function(resolve, reject) {
      ...
      let cardData = {};
      let stripeComponentsMapping = that.component.stripe.componentsMapping || {};
      _each(stripeComponentsMapping, (componentKey, stripeKey) => {
        let comp = that.root.getComponent(componentKey);
        if (comp) {
  ...
  onElementCardChange(result) {
    ...
    let changed = result.complete || this.lastResult && (!!this.lastResult.error != !!result.error) || this.lastResult && this.lastResult.error && result.error && this.lastResult.error.code != result.error.code || false;
